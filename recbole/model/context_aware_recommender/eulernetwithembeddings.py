...
# UPDATE: Modified to support pre-computed embeddings
# @Time   : 2024
# @Author : Modified for embedding support

r"""
EulerNet with Pre-computed Embeddings Support
################################################
Reference:
...
This version extends EulerNet to support pre-computed embeddings (e.g., from text encoders).
Simply change the base class from ContextRecommender to ContextRecommenderWithEmbeddings.
"""
...
class EulerNetWithEmbeddings(ContextRecommenderWithEmbeddings):
    ...
    This version supports pre-computed embeddings via ContextRecommenderWithEmbeddings.
    """
...
class EulerInteractionLayer(nn.Module):
    ...
    def forward(self, complex_features):
        ...
        r, p = r.reshape(r.shape[0], -1), p.reshape(r.shape[0], -1)
